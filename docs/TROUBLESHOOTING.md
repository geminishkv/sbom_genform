...
### Dependency-Check долго качает базу NVD / «зависает» на несколько минут

Каталог `DEP_CHECK_DATA` по умолчанию — `.dependency-check-data`, и это **относительный путь от текущей папки** (cwd). Если запустить `secsbom` не из корня проекта (например, из `/tmp`), существующая база NVD не найдётся, и Dependency-Check будет качать её с нуля (~200+ МБ, несколько минут; без `NVD_API_KEY` — заметно дольше).

Что делать:

- запускайте `secsbom` **из корня проекта**, где лежит `.dependency-check-data` — в логе depcheck подтвердит: `NVD-база: …/odc.mv.db (… МБ) — обновление существующей`;
- либо задайте абсолютный путь: `export DEP_CHECK_DATA=/полный/путь/.dependency-check-data`;
- для скорости задайте `NVD_API_KEY` — обновление базы идёт значительно быстрее.

> То же касается других кэшей: `.bdu_cache` (CVE → BDU) и кэш NVD API — все они относительны cwd.
