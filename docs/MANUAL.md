...
> **Важно — пути относительны текущей папки (cwd).** `secgensbom_out/`, `secgensbom_reports/` и кэши (`.dependency-check-data`, `.bdu_cache`) создаются относительно той папки, **откуда запущен** `secsbom`. Запускайте инструмент **из корня проекта** — тогда отчёты окажутся там, где ожидаете, а Dependency-Check переиспользует существующую базу NVD (иначе скачает её заново — несколько минут). Альтернатива — абсолютные пути через `-o` / `--reports-dir` / `DEP_CHECK_DATA`.
...
- **Docker-образ** — работает от непривилегированного пользователя `sbom` (uid 1001); в `docker-compose.yml` сервисам заданы `no-new-privileges` и `cap_drop: ALL`; встроен `HEALTHCHECK`, OCI-лицензия Apache-2.0.
